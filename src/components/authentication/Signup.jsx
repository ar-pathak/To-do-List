import { IoMdLock } from "react-icons/io";
import { FaUser } from "react-icons/fa";
import { MdPersonalInjury, MdEmail, MdOutlineLock } from "react-icons/md";

const Signup = () => {
  const onSignup = (e) => {
    e.preventDefault();
    console.log("signed up");
  };

  return (
    <div className=" flex items-center justify-center min-h-screen bg-[#FF6767] bgImg-auth md:p-24 sm:p-10 p-5">
      <div className="flex items-center justify-center flex-wrap bg-amber-50 w-full">
        <div className="w-1/2 ">
          <div className=" w-full items-center justify-start xl:flex hidden relative top-20">
            <img src="./img/R 2.png" className="mt-10  " alt="Signup-Img" />
          </div>
        </div>
        <div className="xl:w-1/2 w-full flex flex-col items-center justify-center my-5">
          <div className="w-full md:px-25 px-10 lg:pt-20 md:pt-15 pt-5">
            <h1 className="text-4xl font-bold my-5">Sign In</h1>
            <form onSubmit={onSignup}>
              <div>
                <label
                  htmlFor="firstName"
                  className="flex sm:h-15 h-10 border border-[#565454] rounded-2xl items-center p-3 my-4"
                >
                  <MdPersonalInjury className="h-6 w-6"/>
                  <input
                    type="text"
                    name="firstName"
                    id="firstName"
                    placeholder="Enter First Name"
                    className="outline-none mx-3 "
                  />
                </label>
                <label
                  htmlFor="lastName"
                  className="flex sm:h-15 h-10 border border-[#565454] rounded-2xl items-center p-3 my-4"
                >
                  <MdPersonalInjury className="h-6 w-6"/>
                  <input
                    type="text"
                    name="lastName"
                    id="lastName"
                    placeholder="Enter Last Name"
                    className="outline-none mx-3 "
                  />
                </label>
                <label
                  htmlFor="userName"
                  className="flex sm:h-15 h-10 border border-[#565454] rounded-2xl items-center p-3 my-4"
                >
                  <FaUser className="h-5 w-5"/>
                  <input
                    type="text"
                    name="userName"
                    id="userName"
                    placeholder="Enter Username"
                    className="outline-none mx-3 "
                  />
                </label>
                <label
                  htmlFor="email"
                  className="flex sm:h-15 h-10 border border-[#565454] rounded-2xl items-center p-3 my-4"
                >
                  <MdEmail className="h-5 w-5"/>
                  <input
                    type="email"
                    name="email"
                    id="email"
                    placeholder="Enter Email"
                    className="outline-none mx-3 "
                  />
                </label>
                <label
                  htmlFor="userName"
                  className="flex sm:h-15 h-10 border border-[#565454] rounded-2xl items-center p-3 my-4"
                >
                  <IoMdLock className="h-6 w-6"/>
                  <input
                    className="outline-none mx-3"
                    type="text"
                    name="password"
                    id="password"
                    placeholder="Enter Password"
                  />
                </label>
                <label
                  htmlFor="confirmPassword"
                  className="flex sm:h-15 h-10 border border-[#565454] rounded-2xl items-center p-3 my-4"
                >
                  <MdOutlineLock className="h-6 w-6"/>
                  <input
                    className="outline-none mx-3"
                    type="text"
                    name="confirmPassword"
                    id="confirmPassword"
                    placeholder="Confirm Password"
                  />
                </label>
              </div>
              <div>
                <label className="flex my-5 ">
                  <input
                    type="checkbox"
                    name="rememberMe"
                    id="rememberMe"
                    className="sm:w-8 w-4"
                  />
                  <p className="mx-3">I agree to all terms</p>
                </label>
                <button
                  type="submit"
                  className="sm:py-4 py-2 sm:w-30 w-20 bg-[#FF9090] text-[#F8F9FB] hover:cursor-pointer"
                >
                  Register
                </button>
              </div>
            </form>
          </div>
          <div className="w-full md:px-25 px-10  md:py-10 py-5">
            <div className="flex flex-wrap items-center my-3">
              <p className="mr-2">Already have an account?</p>
              <a href="#" className="text-blue-400 font-bold ">
                Log In
              </a>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};

export default Signup;
